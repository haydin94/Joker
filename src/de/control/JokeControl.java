...
public class JokeControl {
    ...
    public ArrayList<DtoCardJokeTC> getAllJokesView(String category, int start, int count) throws DatabaseException, SqlQueryException, EmptyResultException {
        ViewFactory factory = ViewFactory.getInstance();
        
        ArrayList<DtoCardJokeTC> result = factory.createAllJokesView(category, start, count);
        if (result == null || result.isEmpty()) {
            throw new de.services.exceptions.EmptyResultException("The Query didn´t delivered any result!");
        }
        return result;
    }

    public DtoJokeView getJokeView(int jokeId, boolean justComments, int start, int count) throws DatabaseException, SqlQueryException, EmptyResultException {
        ViewFactory factory = ViewFactory.getInstance();
